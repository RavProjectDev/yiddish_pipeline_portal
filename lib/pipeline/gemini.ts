...
export async function alignGoldToSrt(goldWords: string[], srtEntries: Array<{ index: string; text: string }>) {
  ...
  const systemPrompt =
    ...
    "Output exactly one JSON object where keys are SRT entry numbers as strings, and values are arrays of integers.\n" +
    "Rules:\n" +
    ...
    "3) If a line has no gold words use [].\n" +
    "4) Output JSON only, no markdown/commentary.";
